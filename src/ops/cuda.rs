    ...
    if vars.mask(self.data.val.var()) {
      ...
      if reader.downcast_mut::<NullIo>().is_some() {
        ...
        for idx in 0 .. batch_sz {
          let src_buf = &src_bufs[idx];
          let cache_buf = src_buf.buffer.lock().unwrap();
          val.copy_one(idx, cache_buf.as_ref().slice(src_buf.offset, src_buf.offset + src_buf.size), DeviceStream::implicit().conn());
        }
        offset += val.stride() * val.batch_size();
      } else {
        panic!("store: unimplemented reader type: {:?}", reader);
...
impl CastExt<DeviceBatchArray1d<u8>, DeviceBatchArray1d<f32>> for Rc<AVar<AData<DeviceBatchArray1d<u8>>>> {
...
impl CastExt<DeviceBatchArray3d<u8>, DeviceBatchArray3d<f32>> for Rc<AVar<AData<DeviceBatchArray3d<u8>>>> {
...
impl FlattenExt<DeviceBatchArray3d<f32>, DeviceBatchArray1d<f32>> for Rc<AVar<AData<DeviceBatchArray3d<f32>>>> {
...
impl ReshapeExt<usize, DeviceBatchIoMem<u8>, DeviceBatchArray1d<u8>> for Rc<AVar<AData<DeviceBatchIoMem<u8>>>> {
...
impl AVar<AData<DeviceBatchArray1d<u8>>> for TransformOp<DeviceBatchIoMem<u8>, DeviceBatchArray1d<u8>, ReshapeTransform<usize>> {
  ...
  fn _make_tangent(&self) -> Rc<AVar<AData<DeviceBatchArray1d<u8>>>> {
    self.x_.tangent().reshape(self.kernel.dim)
  }

  fn tangent(&self) -> Rc<AVar<AData<DeviceBatchArray1d<u8>>>> {
    if self.tng_.borrow().is_none() {
      *self.tng_.borrow_mut() = Some(self._make_tangent());
    }
    self.tng_.borrow().as_ref().unwrap().clone()
...
impl ReshapeExt<(usize, usize, usize), DeviceBatchIoMem<u8>, DeviceBatchArray3d<u8>> for Rc<AVar<AData<DeviceBatchIoMem<u8>>>> {
...
impl AVar<AData<DeviceBatchArray3d<u8>>> for TransformOp<DeviceBatchIoMem<u8>, DeviceBatchArray3d<u8>, ReshapeTransform<(usize, usize, usize)>> {
  ...
  fn _make_tangent(&self) -> Rc<AVar<AData<DeviceBatchArray3d<u8>>>> {
    self.x_.tangent().reshape(self.kernel.dim)
  }

  fn tangent(&self) -> Rc<AVar<AData<DeviceBatchArray3d<u8>>>> {
    if self.tng_.borrow().is_none() {
      *self.tng_.borrow_mut() = Some(self._make_tangent());
    }
    self.tng_.borrow().as_ref().unwrap().clone()
...
impl AOp for JoinOp<DeviceIoBatch<f32>, SumJoinKernel> {
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.y.val.overwrite(txn, node) {
      ...
      self.y.val.get_excl(txn, node).set_batch_size(batch_sz0).as_mut()
        .copy(self.xs[0].val.get(txn, node).as_ref(), DeviceStream::implicit().conn());
      for x in self.xs.iter().skip(1) {
        let batch_sz = x.val.get(txn, node).batch_size();
        // FIXME FIXME FIXME: if this gets triggered prob bug somewhere, but ignore for now.
        assert_eq!(batch_sz0, batch_sz);
        self.y.val.get_excl(txn, node).as_mut().flatten_mut()
          .add(1.0, x.val.get(txn, node).as_ref().flatten(), DeviceStream::implicit().conn());
      }
    }
  }
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    for x in self.xs.iter() {
      let batch_sz = x.val.get(txn, node).batch_size();
      if x.grad.accumulate(txn, node, |grad| grad.set_batch_size(batch_sz).as_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
        x.grad.get_mut(txn, node).as_mut().flatten_mut()
          .add(1.0, self.y.grad.get(txn, node).as_ref().flatten(), DeviceStream::implicit().conn());
      }
    }
  }
...
impl AOp for BroadcastAddOp<DeviceBatchArray1d<f32>, DeviceBatchArray3d<f32>> {
  ...
  fn _forward(&self, txn: TxnId) {
    let node = self._id();
    let batch_sz = self.x.val.get(txn, node).batch_size();
    assert_eq!(batch_sz, self.a.val.get(txn, node).batch_size());
    if self.y.val.overwrite(txn, node) {
      self.y.val.get_excl(txn, node).set_batch_size(batch_sz);
      match self.axes.as_slice() {
        &[0, 1] => {
          ...
          unsafe { arraydiff_cuda_kernel_bcast_add_I1an_I2xyan_O1xyan_fwd_f32(
              spatial_dim,
              chan_dim,
              batch_sz,
    ...
    if self.a.grad.accumulate(txn, node, |grad| grad.as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      match self.axes.as_slice() {
        &[0, 1] => {
          ...
          unsafe { arraydiff_cuda_kernel_bcast_add_I1an_I2xyan_O1xyan_bwd_shift_deterministic_f32(
              spatial_dim,
              chan_dim,
              batch_sz,
              self.y.grad.get(txn, node).as_view().as_ptr(),
    ...
    if self.x.grad.accumulate(txn, node, |grad| grad.set_batch_size(batch_sz).as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      match self.axes.as_slice() {
        &[0, 1] => {
          ...
          self.y.grad.get(txn, node).as_view().wait(&conn);
          self.x.grad.get_mut(txn, node).as_view_mut().wait(&conn);
          unsafe { arraydiff_cuda_kernel_bcast_add_I1an_I2xyan_O1xyan_bwd_input_f32(
              spatial_dim,
              chan_dim,
              batch_sz,
              self.y.grad.get(txn, node).as_view().as_ptr(),
...
impl MultExt<DeviceArray2d<f32>, DeviceArray1d<f32>, DeviceBatchArray1d<f32>, DeviceBatchArray1d<f32>> for Rc<AVar<AData<DeviceArray2d<f32>>>> {
  ...
  fn mult_add(&self, x_: Rc<AVar<AData<DeviceBatchArray1d<f32>>>>, b_: Rc<AVar<AData<DeviceArray1d<f32>>>>) -> Rc<LinearOp<DeviceArray2d<f32>, DeviceArray1d<f32>, DeviceBatchArray1d<f32>, DeviceBatchArray1d<f32>>> {
    ...
    LinearOp::new(self.clone(), x_.clone(), Some(b_.clone()), /*clk_horizon,*/ {
      ...
      Rc::new(move |txn, node| {
        let a_dim = a.val.get(txn, node).dim();
        let b_dim = b.val.get(txn, node).dim();
        let x_dim = x.val.get(txn, node).dim();
        assert_eq!(a_dim.1, x_dim);
        assert_eq!(b_dim, x_dim);
        let batch_cap = x.val.get(txn, node).batch_capacity();
        DeviceBatchArray1d::zeros(a_dim.0, batch_cap, DeviceStream::implicit().conn())
      })
    })
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.y.val.overwrite(txn, node) {
      ...
      if let Some(ref b) = self.b {
        ...
        self.x.val.get(txn, node).as_view().wait(&conn);
        self.y.val.get_excl(txn, node).as_view_mut().wait(&conn);
        unsafe { arraydiff_cuda_kernel_bcast_add_I1a_I2an_O1an_fwdaccum_f32(
            chan_dim,
            batch_sz,
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if let Some(ref b) = self.b {
      if b.grad.accumulate(txn, node, |grad| grad.as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
        ...
        if GLOBAL_CONFIG.with(|cfg| cfg.deterministic) {
          unsafe { arraydiff_cuda_kernel_bcast_add_I1a_I2an_O1an_bwd_shift_deterministic_f32(
              chan_dim,
              batch_sz,
              self.y.grad.get(txn, node).as_view().as_ptr(),
          ...
          // FIXME: nondeterministic (atomics) implementation.
          unsafe { arraydiff_cuda_kernel_bcast_add_I1a_I2an_O1an_bwd_shift_deterministic_f32(
              chan_dim,
              batch_sz,
              self.y.grad.get(txn, node).as_view().as_ptr(),
...
impl AOp for ElemLinearOp<f32, DeviceIoBatch<f32>, BroadcastMultAddKernel> {
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.y.val.overwrite(txn, node) {
      self.y.val.get_excl(txn, node).set_batch_size(batch_sz);
      self.y.val.get_excl(txn, node).as_mut()
        .flatten_mut()
        .copy(self.x.val.get(txn, node).as_ref().flatten(), DeviceStream::implicit().conn());
      self.y.val.get_excl(txn, node).as_mut()
        .flatten_mut()
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if self.x.grad.accumulate(txn, node, |grad| grad.as_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      ...
      self.x.grad.get_mut(txn, node).as_mut()
        .flatten_mut()
        .accumulate_scale_constant(
            *self.a.val.get(txn, node),
            self.y.grad.get(txn, node).as_ref().flatten(),
            1.0,
            DeviceStream::implicit().conn());
    }
  }
...
impl<Op> ElemMultExt<DeviceIoBatch<f32>, DeviceIoBatch<f32>> for Rc<Op> where Op: 'static + AVar<AData<DeviceIoBatch<f32>>> {
  fn elem_mult(&self, x_: Rc<AVar<AData<DeviceIoBatch<f32>>>>) -> Rc<ElemLinearOp<DeviceIoBatch<f32>, DeviceIoBatch<f32>, BroadcastMultAddKernel>> {
    ...
    ElemLinearOp::new(self.clone(), x_.clone(), None, BroadcastMultAddKernel, /*clk_horizon,*/ {
      ...
      Rc::new(move |txn, node| {
        let batch_cap = x.val.get(txn, node).batch_capacity();
        assert_eq!(batch_cap, a.val.get(txn, node).batch_capacity());
        DeviceIoBatch::zeros(batch_cap, DeviceStream::implicit().conn())
      })
    })
  }
...
impl AOp for ElemLinearOp<DeviceIoBatch<f32>, DeviceIoBatch<f32>, BroadcastMultAddKernel> {
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.y.val.overwrite(txn, node) {
      ...
      self.y.val.get_excl(txn, node).as_mut()
        .flatten_mut()
        .elem_mult(self.a.val.get(txn, node).as_ref().flatten(), DeviceStream::implicit().conn());
      if let Some(ref b) = self.b {
        // TODO
        unimplemented!();
      }
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if self.x.grad.accumulate(txn, node, |grad| grad.as_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      ...
      self.x.grad.get_mut(txn, node).as_mut()
        .flatten_mut()
        .accumulate_elem_mult(
            1.0,
            self.a.val.get(txn, node).as_ref().flatten(),
            self.y.grad.get(txn, node).as_ref().flatten(),
            1.0,
            DeviceStream::implicit().conn());
    }
    if self.a.grad.accumulate(txn, node, |grad| grad.as_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      self.a.grad.get_mut(txn, node).set_batch_size(batch_sz);
      self.a.grad.get_mut(txn, node).as_mut()
        .flatten_mut()
        .accumulate_elem_mult(
            1.0,
            self.x.val.get(txn, node).as_ref().flatten(),
            self.y.grad.get(txn, node).as_ref().flatten(),
            1.0,
            DeviceStream::implicit().conn());
    }
...
impl ElemMultExt<f32, DeviceBatchArray1d<f32>> for Rc<AVar<AData<f32>>> {
...
impl AVar<AData<DeviceBatchArray1d<f32>>> for ElemLinearOp<f32, DeviceBatchArray1d<f32>, BroadcastMultAddKernel> {
  fn _make_tangent(&self) -> Rc<AVar<AData<DeviceBatchArray1d<f32>>>> {
    let tng_a_ = self.a_.tangent();
    let tng_b_ = self.b_.as_ref().map(|b_| b_.tangent());
    let tng_x_ = self.x_.tangent();
    let tng_y_ = match tng_b_ {
      None          => self.a_.elem_mult(tng_x_).add(tng_a_.elem_mult(self.x_.clone())),
      Some(tng_b_)  => self.a_.elem_mult(tng_x_).add(tng_a_.elem_mult_add(self.x_.clone(), tng_b_)),
    };
    tng_y_
  }
}
...
impl ElemMultExt<f32, DeviceBatchArray3d<f32>> for Rc<AVar<AData<f32>>> {
...
impl AVar<AData<DeviceBatchArray3d<f32>>> for ElemLinearOp<f32, DeviceBatchArray3d<f32>, BroadcastMultAddKernel> {
  fn _owned_data(&self) -> &AData<DeviceBatchArray3d<f32>> {
    &self.y
  }
...
impl ConvExt<(usize, usize), DeviceArray4d<f32>, DeviceArray1d<f32>, DeviceBatchArray3d<f32>, CudnnConvBackend> for Rc<AVar<AData<DeviceArray4d<f32>>>> {
...
impl AVar<AData<DeviceBatchArray3d<f32>>> for ConvOp<(usize, usize), DeviceArray4d<f32>, DeviceArray1d<f32>, DeviceBatchArray3d<f32>, CudnnConvBackend> {
  ...
  fn _make_tangent(&self) -> Rc<AVar<AData<DeviceBatchArray3d<f32>>>> {
    ...
    let tng_x_ = self.x_.tangent();
    let tng_y_ = match tng_b_ {
      None          => self.a_.conv(shape.clone(), tng_x_).add(tng_a_.conv(shape.clone(), self.x_.clone())),
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.y.val.overwrite(txn, node) {
      ...
      if !sizes.contains_key(&batch_sz) {
        ...
        let bwd_w = if GLOBAL_CONFIG.with(|cfg| cfg.deterministic) {
        ...
        let bwd_d = if GLOBAL_CONFIG.with(|cfg| cfg.deterministic) {
      ...
      if let Some(ref b) = self.b {
        ...
        self.x.val.get(txn, node).as_view().wait(&conn);
        self.y.val.get_excl(txn, node).as_view_mut().wait(&conn);
        unsafe { arraydiff_cuda_kernel_bcast_add_I1a_I2xyan_O1xyan_fwdaccum_f32(
            spatial_dim,
            chan_dim,
            batch_sz,
            b.val.get(txn, node).as_view().as_ptr(),
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if let Some(ref b) = self.b {
      if b.grad.accumulate(txn, node, |grad| grad.as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
        ...
        b.grad.get_mut(txn, node).as_view_mut().wait(&conn);
        if GLOBAL_CONFIG.with(|cfg| cfg.deterministic) {
          unsafe { arraydiff_cuda_kernel_bcast_add_I1a_I2xyan_O1xyan_bwd_shift_deterministic_f32(
              spatial_dim,
              chan_dim,
              batch_sz,
              self.y.grad.get(txn, node).as_view().as_ptr(),
  ...
  fn batch_stats(reduce_axes: Axes<(usize, usize)>, cfg: BatchStatsConfig, ctrl: &mut BatchStatsControl, x_: Rc<Op>) -> BatchStatsOutput<DeviceArray1d<f32>> {
    ...
    BatchStatsOp::<(usize, usize), DeviceBatchArray3d<f32>, DeviceArray1d<f32>>::new(reduce_axes, cfg, ctrl, x_.clone(), /*clk_horizon,*/ {
      ...
      Rc::new(move |txn, node| {
        ...
        x.val.rollover(txn, &mut x._vars());
        let x_dim = x.val.get(txn, node).dim();
        match reduce_axes {
...
impl AOp for Softmax2Loss<DeviceBatchArray1d<f32>, DeviceBatchArray1d<f32>, DeviceIoBatch<f32>, KL2Link> {
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    let x_dim = self.x.val.get(txn, node).dim();
    let batch_sz = self.x.val.get(txn, node).batch_size();
    assert_eq!(batch_sz, self.t.val.get(txn, node).batch_size());
    if self.loss.val.overwrite(txn, node) {
      self.loss.val.get_excl(txn, node).set_batch_size(batch_sz);
...
impl AOp for SoftmaxTangent2Loss<DeviceBatchArray1d<f32>, DeviceBatchArray1d<f32>, DeviceIoBatch<f32>, KL2Link> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    assert_eq!(batch_sz, self.t.val.get(txn, node).batch_size());
    assert_eq!(batch_sz, self.prob.val.get(txn, node).batch_size());
    if self.loss_tng.val.overwrite(txn, node) {
      self.loss_tng.val.get_excl(txn, node).set_batch_size(batch_sz);
      assert!(self.prob_tng.val.overwrite(txn, node));
      self.prob_tng.val.get_excl(txn, node).set_batch_size(batch_sz);
      if x_dim <= 1024 {
        let conn = DeviceStream::implicit().conn();
        self.x.val.get(txn, node).as_view().wait(&conn);
...
impl AVar<(AData<DeviceBatchArray1d<f32>>, AData<DeviceIoBatch<f32>>)> for Softmax3Loss<DeviceBatchArray1d<f32>, DeviceIoBatch<u32>, DeviceIoBatch<f32>, DeviceIoBatch<f32>, LRLink> {
  fn _owned_data(&self) -> &(AData<DeviceBatchArray1d<f32>>, AData<DeviceIoBatch<f32>>) {
    unreachable!();
  }
...
impl AOp for Softmax3Loss<DeviceBatchArray1d<f32>, DeviceIoBatch<u32>, DeviceIoBatch<f32>, DeviceIoBatch<f32>, LRLink> {
  ...
  fn _forward(&self, txn: TxnId) {
    let lr_clip = match self.link.lr_clip {
      LRClip::No => f32::INFINITY,
      LRClip::Fixed(clip) => clip as f32,
    };
    let node = self._id();
    let x_dim = self.x.val.get(txn, node).dim();
    let batch_sz = self.x.val.get(txn, node).batch_size();
    assert_eq!(batch_sz, self.t1.val.get(txn, node).batch_size());
    assert_eq!(batch_sz, self.t2.val.get(txn, node).batch_size());
    if self.loss.val.overwrite(txn, node) {
      self.loss.val.get_excl(txn, node).set_batch_size(batch_sz);
      assert!(self.prob.val.overwrite(txn, node));
      self.prob.val.get_excl(txn, node).set_batch_size(batch_sz);
      ...
      }
      {
        let conn = DeviceStream::implicit().conn();
        self.prob.val.get_excl(txn, node).as_view().wait(&conn);
        self.t1.val.get(txn, node).as_ref().wait(&conn);
        self.t2.val.get(txn, node).as_ref().wait(&conn);
        self.loss.val.get_excl(txn, node).as_mut().wait(&conn);
        unsafe { arraydiff_cuda_kernel_softmax_lr_loss_fwd_f32(
            x_dim,
            batch_sz,
            self.prob.val.get_excl(txn, node).as_view().as_ptr(),
            self.t1.val.get(txn, node).as_ref().as_ptr(),
            self.t2.val.get(txn, node).as_ref().as_ptr(),
            self.loss.val.get_excl(txn, node).as_mut().as_mut_ptr(),
            lr_clip,
            conn.raw_stream().as_ptr(),
        ) };
        self.prob.val.get_excl(txn, node).as_view().post(&conn);
        self.t1.val.get(txn, node).as_ref().post(&conn);
        self.t2.val.get(txn, node).as_ref().post(&conn);
        self.loss.val.get_excl(txn, node).as_mut().post(&conn);
      }
    }
  }
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if self.x.grad.accumulate(txn, node, |grad| grad.set_batch_size(batch_sz).as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      ...
      self.loss.grad.get(txn, node).as_ref().wait(&conn);
      self.x.grad.get_mut(txn, node).as_view_mut().wait(&conn);
      unsafe { arraydiff_cuda_kernel_softmax_lr_loss_bwd_f32(
          x_dim,
          batch_sz,
          self.prob.val.get_excl(txn, node).as_view().as_ptr(),
          self.t1.val.get(txn, node).as_ref().as_ptr(),
...
impl AOp for SoftmaxLoss<DeviceBatchArray1d<f32>, DeviceIoBatch<u32>, DeviceIoBatch<f32>, NLLLossLink> {
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    if self.loss.val.overwrite(txn, node) {
      ...
      self.loss.val.get_excl(txn, node).set_batch_size(batch_sz);
      {
        ...
        unsafe { arraydiff_cuda_kernel_block_softmax_negentropy_loss_fwd_accumulate_f32(
            x_dim,
            batch_sz,
            self.prob.val.get_excl(txn, node).as_view().as_ptr(),
            self.loss.val.get_excl(txn, node).as_mut().as_mut_ptr(),
  ...
  fn _backward(&self, txn: TxnId) {
    ...
    if self.x.grad.accumulate(txn, node, |grad| grad.set_batch_size(batch_sz).as_view_mut().set_constant(0.0, DeviceStream::implicit().conn())) {
      ...
      unsafe { arraydiff_cuda_kernel_block_softmax_negentropy_loss_bwd_f32(
          x_dim,
          batch_sz,
          self.prob.val.get_excl(txn, node).as_view().as_ptr(),
          self.loss.grad.get(txn, node).as_ref().as_ptr(),
