...
impl<A> DiffOperator for Var<A> {
  default fn _id(&self) -> NodeId {
    self.node_id
  }

  default fn _push(&self, epoch: Epoch, apply: &mut FnMut(&DiffOperator)) {
    if 1 == self.stack.push(epoch) {
      apply(self);
    }
  }

  default fn _pop(&self, epoch: Epoch, apply: &mut FnMut(&DiffOperator)) {
...
impl DiffOperator for Var<Array1d<f32>> {
  fn _load(&self, txn: TxnId, ref_set: &mut DataRefSet, reader: &mut Any) {
    let node = self._id();
    if self.data.val.write(txn, node, |_| {}) {
...
impl<A, InitF> ArrayOp<A> for InitializeOp<A, InitF> where InitializeOp<A, InitF>: DiffOperator {
  fn data(&self) -> Rc<ArrayData<A>> {
    self.xdat.clone()
  }
}

impl<S, F> DiffOperator for InitializeOp<Array1d<f32, S>, Rc<F>> where S: DerefMut<Target=[f32]>, F: Fn(&mut Array1d<f32, S>) {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _init(&self, txn: TxnId) {
    let node = self._id();
    if self.xdat.val.write(txn, node, |_| {}) {
...
impl<S, MapF> DiffOperator for MapOp<Array1d<f32, S>, MapF> where S: DerefMut<Target=[f32]>, MapF: SpecialMapKernel {
  default fn _id(&self) -> NodeId {
    self.node_id
  }
...
impl<S> DiffOperator for MapOp<Array1d<f32, S>, RectMapKernel> where S: DerefMut<Target=[f32]> {
  fn _forward(&self, txn: TxnId) {
    unimplemented!();
  }
...
impl<S> DiffOperator for TransformOp<Array3d<f32, S>, Array1d<f32, S>, FlattenTransform> where S: DerefMut<Target=[f32]> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _forward(&self, txn: TxnId) {
    let node = self._id();
    let x = self.x.data();
    if self.y.val.write(txn, node, |_| {}) {
  ...
  fn _backward(&self, txn: TxnId, _gauss_newton: bool) {
    let node = self._id();
    let x = self.x.data();
    if x.grad.write(txn, node, |grad| grad.as_view_mut().set_constant(0.0)) {
  ...
  fn _r_forward(&self, txn: TxnId, _gauss_newton: bool) {
    let node = self._id();
    let x = self.x.data();
    if self.y.r_val.write(txn, node, |_| {}) {
  ...
  fn _r_backward(&self, txn: TxnId) {
    let node = self._id();
    let x = self.x.data();
    if x.r_grad.write(txn, node, |r_grad| r_grad.as_view_mut().set_constant(0.0)) {
...
impl<S> DiffOperator for JoinOp<Array1d<f32, S>, SumJoinKernel> where S: DerefMut<Target=[f32]> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
...
impl<S> DiffScalarLoss for LinearOp<Array1d<f32, S>, Array1d<f32, S>, f32, f32> where S: DerefMut<Target=[f32]> {
  fn _set_source(&self, txn: TxnId) {
    let node = self._id();
    //println!("DEBUG: LinearOp: set source");
    assert!(self.y.grad.write(txn, node, |g| *g = 1.0));
  }
}

impl<S> DiffOperator for LinearOp<Array1d<f32, S>, Array1d<f32, S>, f32, f32> where S: DerefMut<Target=[f32]> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _backward(&self, txn: TxnId, _gauss_newton: bool) {
    ...
    let a = self.a.data();
    let x = self.x.data();
    if a.grad.write(txn, node, |grad| grad.as_view_mut().set_constant(0.0)) {
    ...
    if let Some(ref b) = self.b {
      let b = b.data();
      if b.grad.write(txn, node, |g| *g = 0.0) {
...
impl<S> DiffOperator for LinearOp<Array2d<f32, S>, Array1d<f32, S>, Array1d<f32, S>, Array1d<f32, S>> where S: DerefMut<Target=[f32]> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _backward(&self, txn: TxnId, _gauss_newton: bool) {
    ...
    if let Some(ref b) = self.b {
      let b = b.data();
      if b.grad.write(txn, node, |grad| grad.as_view_mut().set_constant(0.0)) {
  ...
  fn _r_backward(&self, txn: TxnId) {
    ...
    let a = self.a.data();
    let x = self.x.data();
    if a.r_grad.write(txn, node, |r_grad| r_grad.as_view_mut().set_constant(0.0)) {
      let x_dim = x.val.get(txn, node).dim();
    ...
    if let Some(ref b) = self.b {
      let b = b.data();
      if b.r_grad.write(txn, node, |r_grad| r_grad.as_view_mut().set_constant(0.0)) {
...
impl DiffScalarLoss for BatchJoinOp<Batch<f32>, f32, SumJoinKernel> {
  fn _set_source(&self, txn: TxnId) {
    let node = self._id();
    assert!(self.y.grad.write(txn, node, |g| *g = 1.0));
  }
...
impl DiffOperator for BatchJoinOp<Batch<f32>, f32, SumJoinKernel> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _backward(&self, txn: TxnId, _gauss_newton: bool) {
    ...
    if x.grad.write(txn, node, |grad| grad.reshape_mut(batch_sz).set_constant(0.0)) {
...
/*impl DiffScalarLoss for LstSqLoss<f32> {
  fn _set_source(&self, txn: TxnId) {
    let node = self._id();
    assert!(self.y.grad.write(txn, node, |g| *g = 1.0));
  }
...
impl DiffOperator for LstSqLoss<Batch<f32>> {
  fn _id(&self) -> NodeId {
    self.node_id
  }
  ...
  fn _forward(&self, txn: TxnId) {
    ...
    let batch_sz = x.val.get(txn, node).batch_size();
    if self.loss.val.write(txn, node, |_| {}) {
  ...
  fn _backward(&self, txn: TxnId, gauss_newton: bool) {
    ...
    if gauss_newton {
      ...
      if x.grad.write(txn, node, |grad| grad.reshape_mut(batch_sz).set_constant(0.0)) {
        ...
        // FIXME(20170209): mix with `loss.grad`.
...
impl<S, Target, Link> DiffOperator for SoftmaxLoss<BatchArray1d<f32, S>, Target, Link> where S: DerefMut<Target=[f32]>, Link: LikelihoodLossLink {
  default fn _id(&self) -> NodeId {
    self.node_id
  }
...
impl<S> DiffOperator for SoftmaxLoss<BatchArray1d<f32, S>, BatchArray1d<f32, S>, KL2LossLink> where S: DerefMut<Target=[f32]> {
  fn _forward(&self, txn: TxnId) {
    unimplemented!();
  }
...
impl<S> DiffOperator for SoftmaxLoss<BatchArray1d<f32, S>, Vec<(u32, f32)>, LRLossLink> where S: DerefMut<Target=[f32]> {
  fn _forward(&self, txn: TxnId) {
    unimplemented!();
  }
...
impl<S> DiffOperator for SoftmaxLoss<BatchArray1d<f32, S>, Vec<u32>, NLLLossLink> where S: DerefMut<Target=[f32]> {
  fn _forward(&self, txn: TxnId) {
    unimplemented!();
  }
