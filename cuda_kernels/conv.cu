...
__global__ void conv_bcast_add_fwd_f32_kernel(
...
extern "C" void arraydiff_cuda_kernel_conv_bcast_add_fwd_f32(
    size_t spatial_dim,
    size_t chan_dim,
    size_t batch_sz,
    const float *x,
    ...
    cudaStream_t stream)
{
  ...
  conv_bcast_add_fwd_f32_kernel<<<(n+1024-1)/1024, 1024, 0, stream>>>(
      spatial_dim, chan_dim, batch_sz, x, shift, y);
}

__global__ void conv_bcast_add_param_bwd_nonatomic_f32_kernel(
    uint32_t num_rounds,
    uint32_t spatial_dim,
    uint32_t chan_dim,
    uint32_t batch_sz,
    ...
    float *shift_grad)
{
  ...
  if (chan_idx < chan_dim && batch_idx < batch_sz) {
    for (uint32_t round = 0; round < num_rounds; round++) {
      ...
      //uint32_t block_dim = min(blockDim.x, spatial_dim - round_offset);
      uint32_t round_idx = round_offset + threadIdx.x;
      uint32_t spatial_idx = round_idx;
...
extern "C" void arraydiff_cuda_kernel_conv_bcast_add_param_bwd_nonatomic_f32(
    size_t spatial_dim,
    size_t chan_dim,
    size_t batch_sz,
    const float *y_grad,
    ...
    cudaStream_t stream)
{
  uint32_t num_rounds = (spatial_dim + 1024-1) / 1024;
  uint32_t num_blocks = chan_dim * batch_sz;
  conv_bcast_add_param_bwd_nonatomic_f32_kernel<<<num_blocks, 1024, 0, stream>>>(
      num_rounds, spatial_dim, chan_dim, batch_sz, y_grad, shift_grad);
}

__global__ void conv_bcast_add_input_bwd_f32_kernel(
    uint32_t spatial_dim,
    uint32_t chan_dim,
    uint32_t batch_sz,
    const float *y_grad,
...
extern "C" void arraydiff_cuda_kernel_conv_bcast_add_input_bwd_f32(
    size_t spatial_dim,
    size_t chan_dim,
    size_t batch_sz,
    const float *y_grad,
...
{
  uint32_t n = spatial_dim * chan_dim * batch_sz;
  conv_bcast_add_input_bwd_f32_kernel<<<(n+1024-1)/1024, 1024, 0, stream>>>(
      spatial_dim, chan_dim, batch_sz, y_grad, x_grad);
}
